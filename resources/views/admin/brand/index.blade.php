                ...
                <div class="card">
                    <div class="container mt-4">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h2 class="h4"><i class="bi bi-grid me-2"></i>Brand List</h2>
                                                ...
                                                <tr class="text-center" id="chart">
                                                    ...
                                                    <td class="text-center align-middle">
                                                        {{-- <img src="{{ asset('storage/' . $brand->image) }}"
                                                            ...
                                                            alt="Product"> --}}
                                                        {{-- <img src="{{ asset('storage/' . $brand->image) }}"
                                                            alt="{{ $brand->name }}" style="width: 100px; height: auto;"
                                                            class="img-thumbnail"> --}}

                                                        <img src="{{ Storage::url($brand->image) }}" style="width: 100px;"
                                                            alt="{{ $brand->name }}">
                                                    </td>
                                                    <td class="text-start">
                                                        <strong>{{ $brand->name }}</strong>
                                                    </td>
                                                    <td class="text-start"><strong>{{ $brand->slug }}</strong></td>
                                                    <td class="text-start"><strong> {{ $brand->slug }} </strong>
                                                    </td>
                                                    <td>
                                                        <div class="btn-group" role="group">
                                                            ...
                                                            <form action="{{ route('brands.destroy', $brand) }}"
                                                                method="POST" id="delete-form-{{ $brand }}">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="button"
